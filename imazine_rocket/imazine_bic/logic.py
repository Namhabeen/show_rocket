...
class Sign:
    ...
    def login(request, id,pwd):#로그인
        users = User.objects.filter(id = id)
        response = render(request, 'imazine_bic/login.html',{"okay":1,"users":users})
        if users is not None:
            ...
            response = HttpResponse("<html><script>alert('로그인 오류입니다. 다시 시도해주세요');location.href='login';</script></html>")
        return response
    ...
    def signup(request,user_info): #회원가입
        ...
        if user_info == "0":
            return render(request, 'imazine_bic/login.html')
